...
class IronBankCog(commands.Cog):
    ...
    def calculate_max_credit(self, user: User) -> int:
        ...
        # Base Calculation
        base_limit = self.BASE_CREDIT + (trust * self.TRUST_FACTOR) + (duty * self.DUTY_FACTOR)

        # Risk Multiplier (Sentinel)
        # C_multi = 1 - (Risk * 0.01)
        # Risk 50 => 0.5 (50% limit reduction)
        risk = user.risk_score # Property
        c_multi = max(0.1, 1.0 - (risk * 0.01)) # Floor at 10%

        return floor(base_limit * c_multi)
