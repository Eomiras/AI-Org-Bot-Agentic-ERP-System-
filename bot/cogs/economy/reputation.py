        ...
        async with AsyncSessionLocal() as session:
            ...
            for user in users:
                ...
                if delta.days > self.INACTIVE_DAYS:
                    # Apply Decay
                    old_trust = user.trust_score or 50
                    old_duty = user.duty_score or 50

                    new_trust = self.calculate_new_score(old_trust, delta.days)
                    new_duty = self.calculate_new_score(old_duty, delta.days)

                    if new_trust != old_trust or new_duty != old_duty:
                        user.trust_score = new_trust
                        user.duty_score = new_duty
                        decay_count += 1
                        # Log/Debug
                        # logger.debug(f"Decayed User {user.discord_id}: Trust {old_trust}->{new_trust}")
