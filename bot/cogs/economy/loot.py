...
# --- 1. The Math Logic (Integer Policy & Risk) ---
class LootCalculator:
    def __init__(self, gross_amount: int, tax_rate: float, participants: int, avg_risk_score: float = 0.0):
        self.gross = gross_amount
        self.tax_rate = tax_rate
        self.count = participants
        self.risk = avg_risk_score

    def calculate(self) -> dict:
        if self.count <= 0: return {}

        # Risk Multiplier: T_multi = 1 + (Risk * 0.005)
        # Risk 50 => 1.25 (25% higher tax rate effectively)
        t_multi = 1.0 + (self.risk * 0.005)
        effective_rate = self.tax_rate * t_multi

        # 1. Calculate Tax (Floor)
        tax_amount = floor(self.gross * effective_rate)
...
class LootCog(commands.Cog):
    ...
    async def split(self, ctx: discord.ApplicationContext, screenshot: discord.Attachment, members: str):
        ...
        # 2. Risk Assessment (Sentinel Integration)
        # Calculate Average Risk of the group (or just the Uploader?)
        # Design says: "Read RS for Transactions". Usually applies to the group leader or averaged.
        # We will use the Uploader's Risk for simplicity and accountability.

        uploader_risk = 0.0
        async with AsyncSessionLocal() as session:
            u_res = await session.execute(select(User).where(User.discord_id == ctx.author.id))
            u = u_res.scalar_one_or_none()
            if u:
                uploader_risk = u.risk_score

        # 3. Calculation
        calc = LootCalculator(gross_amount, self.TAX_RATE, len(participants), avg_risk_score=uploader_risk)
        data = calc.calculate()
