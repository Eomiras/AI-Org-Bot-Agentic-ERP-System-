...
class RSIHandleModal(ui.Modal):
    def __init__(self):
        super().__init__(title="RSI Account Verification")
        self.add_item(ui.InputText(label="RSI Handle", placeholder="YourExactHandleName", required=True))

    async def callback(self, interaction: discord.Interaction):
        handle_text = self.children[0].value.strip()
